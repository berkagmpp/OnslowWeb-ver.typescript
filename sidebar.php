<div class="sidebar">
  <!-- insert your sidebar items here -->
  <h3>Latest News</h3>
  <h4>New Web applicaiton Launched</h4>

  <h3>Useful Links</h3>
  <ul>
    <li><a href="https://openpolytechnic.ac.nz">Open Polytechnic</a></li>
    <li><a href="https://openpolytechnic.iqualify.com/local-login">iQualify</a></li>
    <li><a href="#">no link </a></li>
    <li><a href="/onslow-ts/privacy.php">Privacy statement</a></li>
  </ul>
  <h3>Search</h3>
  <form method="post" action="#" id="search_form">
    <p>
      <input class="search" type="text" name="search_field" value="Enter keywords....." />
      <input name="search" type="image" style="border: 0; margin: 0 0 -9px 5px;" src="/onslow-ts/style/search.png" alt="Search" title="Search" />
    </p>
  </form>
</div>